...
import { DateAdapter, MAT_DATE_FORMATS, MAT_DATE_LOCALE, MatTabGroup, MatPaginator, MatSort, MatTableDataSource, MatTabChangeEvent } from '@angular/material';
...
export class AprobacionTBKComponent implements OnInit {
  ...
  displayedColumnsCarga = ['actions', 'fAlta', 'tipo', 'naviera.nombreComercial', 'cliente.nombreComercial', 'observaciones', 'estatus'];

  dtCargas: any;
  totalRegistrosCargas = 0;

  constructor(public liberacionService: LiberacionBLService,private excelService: ExcelService) { }
  ...
  cargaSolicitudes(CD: string) {
    ...
    if (CD == 'C') {
      this.cargando = true;
      this.liberacionService.getLiberacion('C', '',
        this.fIni ? this.fIni.utc().format('DD-MM-YYYY') : '',
        this.fFin ? this.fFin.utc().format('DD-MM-YYYY') : '', naviera)
        .subscribe(resp => {
  ...
  borrarSolicitud(liberacion: Liberacion) {
    swal({ title: '¿Esta seguro?', text: 'Esta apunto de borrar la solicitud.', icon: 'warning', buttons: true, dangerMode: true, })
      .then(borrar => {
        if (borrar) {
          this.liberacionService.borrarSolicitud(liberacion._id)
            .subscribe(borrado => {
